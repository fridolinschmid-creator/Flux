...
### Erledigt
- [x] Aktions-Audit-Log / Erklaerbarkeit (Ehrlichkeits-Feature) – nachvollziehbares Protokoll ALLER von der KI ausgefuehrten X:-Aktionen (Mail/SMS/Anruf/setting). Neues Modul `fluxai/src/audit.c`+`.h` (`flux_audit_log(type,desc,result)`) schreibt nach AUSGEFUEHRTER Aktion append-only nach `/etc/flux/audit.txt` (`chmod 0600` wie andere /etc/flux-Dateien, Tail-Begrenzung auf 500 Zeilen analog habits.c), Format `[YYYY-MM-DD HH:MM] TYP: Beschreibung -> Ergebnis` (einzeilig, Newlines entschaerft). Verdrahtet in exec.c: jede Mail/SMS/Anruf/Einstellung wird nach Ausfuehrung mit dem EHRLICHEN Ergebnis (auch Fehlschlaege/Stubs, z.B. "SMS: kein Modem erkannt") geloggt. KEINE GEHEIMNISSE: Mail loggt nur Empfaenger+Betreff (NICHT den Mailtext), SMS/Anruf nur den Empfaenger (NICHT den SMS-Text); pin_hash/Passwoerter/API-Keys ohnehin nie im setting-Pfad (Allowlist). Neues Lese-Tool `audit_list` (tools.c) listet die letzten N Eintraege (Default 15, max 50, neueste zuerst), liest AUSSCHLIESSLICH audit.txt (kein beliebiger Pfad), EHRLICH "noch keine KI-Aktionen protokolliert" wenn die Datei fehlt; registriert in flux_tool_exec() + flux_tools_description() (mit Beispiel-Triggern). README-Abschnitt "Aktions-Protokoll (Nachvollziehbarkeit)" ergaenzt (was geloggt wird, was bewusst NICHT). KEIN eigener Screen gebaut (FLUX_SCREEN_AUDIT bewusst weggelassen: haette Enum/State/ui.c-Geometrie/Hit-Test + drei Screen-Wechsel-Pfade in der Shell betroffen und den Rahmen einer minimalen Iteration gesprengt -- das audit_list-Tool deckt den Lesepfad ab; ehrlich so in README vermerkt).  — Compile-Test bestanden (fluxai 15 / shell 31 Warnungen, keine neuen, 0 Fehler), beide Binaries gebaut; End-to-End (echtes Schreiben von audit.txt bei einer bestaetigten Aktion + audit_list-Ausgabe) NICHT in QEMU verifiziert (kein laufender KI-Anbieter im Build/QEMU fuer den ACTION->X:-Pfad) -- die Schreib-/Lese-Logik selbst ist Standard-Datei-I/O wie habits/notes
