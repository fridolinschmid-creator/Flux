...
int main(int argc, char *argv[]) {
    ...
    /* 28b -- Browser: leer (Startzustand) */
    flux_ui_draw_browser(&fb, "", "", 0);
    save_png(&fb, outdir, "28b_browser_leer");

    /* 28c -- Browser: Adresse wird eingetippt */
    flux_ui_draw_browser(&fb, "", "example.com", 0);
    save_png(&fb, outdir, "28c_browser_eingabe");

    /* 28d -- Browser: echte Seite geladen (litehtml-Rendering, echter
     * Netzabruf -- dieses Host-Tool hat wie main.c jetzt eigenes libcurl,
     * siehe browser_render.h). Faellt die Abfrage aus (kein Netz im
     * Build-Sandbox), bleibt der leere Zustand sichtbar -- ehrlich statt
     * erfundener Platzhalterinhalt. */
    {
        char resolved[256] = {0}, title[256] = {0}, err[128] = {0};
        flux_browser_render_open("https://example.com", fb.width,
                                 resolved, sizeof(resolved), title, sizeof(title),
                                 err, sizeof(err));
        flux_ui_draw_browser(&fb, resolved, "", 0);
    }
    save_png(&fb, outdir, "28d_browser_seite");
